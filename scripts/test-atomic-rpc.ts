...
async function runTests() {
  console.log('Running Atomic Transaction RPC Tests...')
  ...
  if (resetErr) console.error('Error resetting balance:', resetErr.message)

  try {
    console.log('\n[1] Simple Buy (10 @ $150, $5 comm)...')
    ...
    if (errors.length > 0) {
        console.log(`   Concurrency lock handled conflict: ${errors[0].error?.message}`)
    } else {
    ...
    console.log('\nAll atomic tests completed successfully.')
