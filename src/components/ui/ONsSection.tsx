...
import { TrendingUp, TrendingDown, Minus, ChevronUp, ChevronDown, Landmark } from 'lucide-react'
...
export default function ONsSection({ positions, loading, summary }: Props) {
    ...
    <div className="glass rounded-2xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.05]">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500 font-mono mt-0.5">{positions.length} posición{positions.length !== 1 ? 'es' : ''} · bonos corporativos USD</p>
          </div>
        </div>
      ...
      <div className="flex flex-col md:hidden">
        <div className="flex items-center px-3 py-2 border-b border-white/[0.06] bg-white/[0.02]">
          <span className="flex-1 text-[9px] font-mono text-slate-500 uppercase tracking-wider">Ticker</span>
          <span className="flex-1 text-right text-[9px] font-mono text-slate-500 uppercase tracking-wider">Invertido</span>
          <span className="flex-1 text-right text-[9px] font-mono text-slate-500 uppercase tracking-wider">P&L USD</span>
          <span className="flex-1 text-right text-[9px] font-mono text-slate-500 uppercase tracking-wider">P&L %</span>
        </div>
          ...
          {sorted.length === 0 ? (
            ...
            return (
              <div key={`mob-on-${pos.id}`}
                className="flex items-center px-3 py-2 animate-fade-in"
                style={{ animationDelay: `${i * 10}ms`, animationFillMode: 'both' }}>
                <div className="flex-1">
                  <span className="text-xs font-semibold text-white">{pos.ticker}</span>
                </div>
                <div className="flex-1 text-right">
                  <span className="text-xs font-mono tabular-nums text-slate-400">{formatUSD(pos.total_invested)}</span>
                </div>
                <div className="flex-1 text-right">
                  <span className={cn('text-xs font-mono tabular-nums', isPosPnl ? 'text-emerald' : isNegPnl ? 'text-rose' : 'text-slate-400')}>
