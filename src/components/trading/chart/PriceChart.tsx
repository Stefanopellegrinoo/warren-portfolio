...
const OPERATION_MARKER_STYLE = {
  COMPRA:    { position: "belowBar" as const, color: "#26a69a", shape: "arrowUp" as const,   text: "C" },
  VENTA:     { position: "aboveBar" as const, color: "#ef5350", shape: "arrowDown" as const, text: "V" },
...
export function PriceChart({ symbol, timeframe }: Props) {
  ...
  const txMarkersRef = useRef<SeriesMarker<UTCTimestamp>[]>([]);
  ...
  useEffect(() => {
    ...
    for (const [id, apiLine] of Array.from(map.entries())) {
      if (!activeIds.has(id)) {
        try {
  ...
  // Portfolio overlay — transaction markers (COMPRA / VENTA / DIVIDENDO)
  useEffect(() => {
    const series = candleSeriesRef.current;
    if (!series) return;
    ...
    function getOrCreatePlugin() {
      if (!markersPluginRef.current) {
        markersPluginRef.current = createSeriesMarkers(series as ISeriesApi<"Candlestick", UTCTimestamp>);
      }
    ...
    function flushMarkers() {
      const combined = [...txMarkersRef.current, ...signalMarkersRef.current];
    ...
    async function loadMarkers() {
      try {
        ...
        txMarkersRef.current = txs
          .filter((tx) => tx.operation === "COMPRA" || tx.operation === "VENTA" || tx.operation === "DIVIDENDO")
          .map((tx) => {
            const time = Math.floor(new Date(tx.date + "T00:00:00Z").getTime() / 1000) as UTCTimestamp;
            const style = OPERATION_MARKER_STYLE[tx.operation];
            return { time, ...style };
          });

        flushMarkers();
      } catch (e) {
        console.error("Portfolio overlay: failed to load transactions", e);
      }
    }

    async function loadSignalMarkers(ticker: string) {
      try {
        const res = await fetch(`/api/signals?ticker=${encodeURIComponent(ticker)}&hours=720`);
      ...
      try {
        if (dataSource === "binance") {
          ...
          const poll = async () => {
            if (cancelled) return;
            try {
              const qRes = await fetch(`/api/quote?ticker=${encodeURIComponent(symbol)}`);
              if (!qRes.ok) return;
              const [quote] = (await qRes.json()) as Array<{ ticker: string; price: number | null }>;
              if (quote?.price) {
                const arr = candlesRef.current;
                const prev = arr[arr.length - 2];
                setLastPrice({
                  value: quote.price,
                  pct: prev && prev.close !== 0 ? ((quote.price - prev.close) / prev.close) * 100 : 0,
                });
              }
            } catch {}
          };
