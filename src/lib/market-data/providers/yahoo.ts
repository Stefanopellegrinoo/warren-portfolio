    ...
    try {
      ...
      const candles: Candle[] = result.map((row: any) => {
        ...
        if (hCcl) {
          ...
          // No `if (rate)` fall-through here — same rule as
          // fetchHistoricalQuotes in yahoo-finance.ts. A missing rate left the
          // whole candle in ARS and handed it back as USD, so the chart drew a
          // day roughly 1000x above its neighbours. Silence is the bug.
          //
          // Thrown as a MarketDataError on purpose: the enclosing catch runs
          // classifyError, which returns a MarketDataError untouched. A plain
          // Error would be re-read by its message and could be reclassified.
          if (!rate) {
            throw new MarketDataError(
              'PROVIDER_UNAVAILABLE',
              `No historical CCL rate for ${ticker} on ${dateStr} — refusing to return an ARS candle as USD`
            )
          }
          open = open / rate
          high = high / rate
